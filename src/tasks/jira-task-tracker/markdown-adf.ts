...
// (docs/layered-body-format.md): headings, paragraphs with **bold** / `code`
// marks, fenced code blocks, task lists, bullet lists, and <details> blocks
// (mapped to ADF expands). Written bodies render as structured content in the
// Jira UI, and reads reconstruct markdown so downstream pipeline steps
...
export function markdownToAdf(markdown: string): AdfDocNode {
...
function parseBlocks(lines: string[]): AdfNode[] {
  ...
  while (i < lines.length) {
    ...
    // Paragraph: consecutive lines up to a blank line or the start of another
    // block, joined with hardBreaks so single newlines survive the round-trip.
    const paragraph: AdfNode[] = []
    while (i < lines.length) {
      const current = lines[i]
      if (current === undefined || current.trim() === '' || startsBlock(current)) break
...
// A <details> block spanning lines[start..]: find the matching </details>
// (nesting-aware), lift the <summary> into the expand title, and parse what
// remains as blocks. Returns undefined for an unclosed block, which then
// falls through to plain paragraph text.
function parseDetails(lines: string[], start: number): { node: AdfNode; next: number } | undefined {
  let depth = 0
  let end = -1
...
function blockToMarkdown(node: AdfNode): string {
  switch (node.type) {
    ...
    case 'taskList':
      return (node.content ?? [])
        .map((item) => `- [${item.attrs?.['state'] === 'DONE' ? 'x' : ' '}] ${inlineToMarkdown(item.content ?? [])}`)
        .join('\n')
    case 'bulletList':
      return (node.content ?? [])
        .map((item) => `- ${adfToMarkdown(item.content ?? []).replace(/\n/g, '\n  ')}`)
        .join('\n')
    case 'orderedList':
      return (node.content ?? [])
        .map((item, index) => `${index + 1}. ${adfToMarkdown(item.content ?? []).replace(/\n/g, '\n   ')}`)
        .join('\n')
    case 'expand': {
    ...
    case 'rule':
      return '---'
    default:
      // Unknown block (table, panel, media, …): flatten to its text so reads
      // never throw on content authored in the Jira UI.
...
function inlineToMarkdown(nodes: AdfNode[]): string {
  return nodes
    .map((node) => {
      ...
      let text = node.text ?? (node.content !== undefined ? inlineToMarkdown(node.content) : '')
